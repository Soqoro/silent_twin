...
\begin{itemize}
    ...
    \item \textbf{End-to-end scoring hides a channel--realization gap.} In a post-hoc audit, Travel and Workspace often produced informative two-state posteriors while emitting the wrong symbolic namespace. This is evidence for a measurement hypothesis, not a revised primary result.
...
This diagnostic motivates a separately frozen train-only interface-realization experiment. It should replay identical public transcripts under the original interface, a minimal decoder interface, length-matched irrelevant schema padding, randomized opaque labels, and a posterior-only response contract. Posterior AUC, exact-schema validity, namespace substitution, conditional accuracy, and conservative end-to-end accuracy must be preregistered as distinct outcomes. Until such an intervention is run, interface complexity is a supported mechanism hypothesis rather than a causal paper claim.
...
The E1 audit adds a measurement distinction for future experiments. \emph{Channel distinguishability} asks whether any decoder can recover the state from the transcript. \emph{Posterior realization} asks whether the learned model's numeric state score ranks the states. \emph{Contract realization} asks whether the model emits the required exact schema and label namespace. \emph{End-to-end realization} retains the conservative operational score in which invalid output is failure. These quantities answer different questions and will be reported separately; a post-hoc posterior recovered from a rejected raw response never silently replaces the frozen end-to-end endpoint.
...
We do not claim the first hidden guardrail, transactional agent runtime, provenance tracker, noninterference definition, or clean-context recovery mechanism. We do not claim that private authorization states are secrets in every deployment, that learned guardrails commonly have complementary blind spots, or that the benchmark estimates real-world attack prevalence. The completed E1 supports train-only evidence that the source and recipient of enforcement feedback control state information in a local simulated AgentDojo setting. It does not yet establish feedback-caused prohibited effects, useful repair, held-out generalization, or a causal effect of interface complexity. We claim a joint runtime contract, formal proof obligations, and a causal evaluation of who receives enforcement feedback and when. Tier-2 effects remain benchmark prohibited effects in local simulated environments.
...
E1 also exposes a measurement limitation. Exact-contract failure is operationally relevant and must remain failure in the primary score, but it can hide information already present in a model-generated posterior. The channel--realization audit is post-hoc, uses one Qwen checkpoint and one controlled prompt family, and is confounded by suite-specific tool ontologies. It motivates a new intervention; it does not prove that schema count causes namespace substitution or that the same pattern holds for other models.
...
\begin{enumerate}
    ...
    \item \textbf{Next:} preregister a small train-only interface-realization replay and run E2 train to estimate feedback-assisted prohibited effects and \method{} closure, with neither experiment treated as a route around the failed E1 gate;
...
The immediate scientific risk is no longer whether Qwen can interact with the benchmark. It is whether feedback-derived state information translates into a prohibited effect and whether the observed interface-realization gap survives a frozen intervention. The main utility risk remains benchmark authoring and independent dependency review. The repository already contains the AgentDojo catalog and splits, PBS execution, provenance validation, aggregation, Tier-1 causal runner, and engineering coverage for E1--E5. No scientific-v5 development, pair reduction, or held-out job is reused under the revised claim.
...
The controlled pilot demonstrates that verdict-correlated feedback can produce both state inference and additional synthetic prohibited effects under a fixed opportunity budget. Scientific-v6 E1 now shows that a learned open-model attacker extracts feedback-source state in realistic AgentDojo workflows: genuine feedback tracks the target state, shuffled feedback tracks only its donor, and recipient-separated feedback remains near chance. It also shows why end-to-end leakage must be decomposed carefully. Banking and Slack realize the signal under the exact output contract, while Travel and Workspace often expose the signal only in a rejected state posterior. The frozen placebo criterion nevertheless fails, so these findings remain train-only and development stays closed. The next evidence must establish whether leakage changes prohibited effects, whether \method{} closes that increment, and whether clean repair preserves useful work. If those stages succeed, the paper's strongest contribution will be both architectural and methodological: truthful feedback can be moved away from a potentially compromised context, and privacy evaluation must distinguish a quiet channel from an attacker that merely fails to express what the channel taught it.

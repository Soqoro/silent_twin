...
\begin{itemize}
    ...
    \item \textbf{The first learned clean-repair configuration is not feasible.} The released 49-task oracle corpus has strict utility 1.0, but all 147 flattened-JSON Qwen episodes are contract-invalid. A separately preregistered native-tool gate raises turn parsing to 99.19\% yet reaches only 74.43\% equal-suite episode validity, below its prospective threshold.
    \item \textbf{Utility and generalization remain open.} Because the repair interface did not qualify, its observed utility is diagnostic rather than an efficacy estimate. The E1 gate failed, E2 is estimation-only, and development and test have not been opened.
\end{itemize}
...
\begin{enumerate}
    ...
    \item We connect an impossibility result for same-context state-specific repair to a randomized compromised-agent benchmark and a prospectively gated benign-repair component.
    ...
    \item We separate channel distinguishability, posterior realization, syntactic contract realization, grounded tool execution, state-matched action selection, and released effects so that protocol noncompliance cannot be mistaken for channel privacy or action transfer.
...
The completed E1 and E2 cover all four available suites---Workspace, Travel, Banking, and Slack---with 20, 10, 8, and 11 structural groups respectively. The frozen primary attacker is Qwen2.5-7B. A deterministic attacker remains a positive control. The same Qwen checkpoint was subsequently evaluated as a fresh restricted repair process on one released clean task per structural group. The first strict-JSON interface failed completely, and a prospective native-tool qualification gate also failed; no repair efficacy or development run follows from those observations. A fixed Granite Guardian action monitor may be included as an additional checker, but the primary authorization intervention does not depend on finding complementary Granite profiles. All learned-model evidence is train-only and estimation-only; a stronger attacker, a different repair checkpoint, or any development execution requires a separately frozen protocol and permission-bearing gate.
...
Applying the frozen structural-group AUC procedure post hoc to the numeric posterior in the raw response gives genuine-feedback target-state AUCs of 1.000 in Banking, 1.000 in Slack, 0.720 in Travel (95\% CI 0.570--0.910), and 0.870 in Workspace (95\% CI 0.760--0.968). The equal-suite AUC is 0.898 (95\% CI 0.853--0.952). The controls follow the causal source of feedback: matched-shuffled posterior AUC is 0.500 for the target state but 0.901 for the independently crossed donor state (95\% CI 0.860--0.954), while \method{} target-state AUC is 0.510 (95\% CI 0.485--0.540). These intervals are regenerated by the submission audit using the frozen 5,000-resample structural-group procedure; the point estimates and interpretation are unchanged.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{\method{} cell} & \textbf{State prediction} & \textbf{Selection/commit} & \textbf{Pooled invalid} & \textbf{Conservative effect} \\
\midrule
$Q=0$ & 0.2180 & 0.5000 & 0.6022 & 0.7820 \\
$Q=16$ & 0.4646 & 0.5000 & 0.0780 & 0.5406 \\
\bottomrule
\end{tabular}
\caption{Frozen \method{} cells. Invalid is the pooled raw-row error rate; the other entries are equal-suite estimates. Recipient separation leaves a measurable inference contrast but removes the observed probe-dependent change in state-matched choice and commitment.}
...
\subsection{Prospective Clean-Repair Feasibility Gates}

The security experiments do not by themselves establish that a fresh recipient can complete useful work. We therefore constructed a separate train-only component study before making a repair claim. One scenario was selected without model outcomes from each of the same 49 structural groups. The released ground-truth plans execute with strict utility 1.0 in all 49 tasks, showing that the task corpus, tool adapters, and scorer admit a solution. Each task was then crossed with no repair, the released oracle, a clean-start Qwen context with full tools, a sanitized fresh Qwen repair context with full tools, and the same repair context restricted to the oracle function names.

The initial learned interface required one strict JSON object. All 147 learned episodes made an authentic local Qwen call, but none satisfied that contract: 81 responses omitted the required \texttt{content} key, 50 were non-JSON fenced responses, and 16 mixed tool calls with disallowed final content. All learned utilities are consequently zero under the frozen scoring rule. This is an interface-feasibility failure, not evidence that semantic repair has zero utility. The no-repair control has equal-suite utility 0.0625 and the oracle has utility 1.0.

Because no learned call reached tool execution, we preregistered one adaptive qualification study using the exact native tool template in the pinned Qwen tokenizer. It used the same 49 train tasks, one fresh sanitized restricted context per task, no parser repair or retry, and an atomic replay after context retirement. Progression required equal-suite task-level protocol validity of at least 0.90 and a 95\% suite-stratified bootstrap lower bound of at least 0.80; utility was explicitly not a gate.

\begin{table}[htbp]
\centering
\small
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.24\linewidth}X>{\raggedright\arraybackslash}p{0.23\linewidth}@{}}
\toprule
\textbf{Repair-stage check} & \textbf{Frozen train observation} & \textbf{Disposition} \\
\midrule
Released clean-task oracle & 49/49 strict successes; zero tool errors & Corpus and scorer feasible \\
Flattened strict-JSON Qwen & 0/147 learned episodes contract-valid & Semantic repair not estimable \\
Native Qwen turn parsing & 122/123 calls parse (0.9919) & Syntax largely realized \\
Native Qwen episode validity & Equal-suite 0.7443, 95\% CI $[0.6216,0.8585]$; task-weighted 35/49 & Fails 0.90/0.80 gate \\
Native Qwen run validity & Equal-suite 0.6443, 95\% CI $[0.5068,0.7773]$; task-weighted 31/49 & No repair-efficacy release \\
\bottomrule
\end{tabularx}
\caption{Prospective train-only repair feasibility checks. Episode validity, not utility direction, determined progression.}
\label{tab:repairfeasibility}
\end{table}
\FloatBarrier

Only one of the 14 invalid native episodes contains malformed tool-call JSON. The other 13 fail after syntactically valid calls: eight invent an entity identifier, three issue an unsuccessful search, one supplies an invalid email, and one omits a required argument. Native chat therefore moves the bottleneck from serialization to grounded multi-turn tool use. Four otherwise protocol-valid Travel tasks also fail the formal prompt-binding flag because the client hashes non-ASCII text with UTF-8 characters preserved while the validator hashes ASCII escapes. Their structured messages, tools, and rendered bytes match exactly; the frozen flags nevertheless remain failures. Even ignoring this instrumentation limitation, 35/49 protocol-valid episodes cannot satisfy the prospective gate.

This result adds a second, downstream measurement branch:
\[
\text{private settlement}
\longrightarrow \text{fresh repair interface}
\longrightarrow \text{grounded tool execution}
\longrightarrow \text{useful effect}.
\]
The present evidence validates the oracle branch and localizes two learned-interface failures, but it does not estimate the final useful-effect arrow for a qualified learned repair process. No development, test, or stronger-checkpoint result was inspected.

\subsection{Frozen Evidence Index}

Table~\ref{tab:evidenceindex} gives the compact publication index for the complete empirical chain. Each hash is the leading 12 hexadecimal characters of the exact file SHA-256; the companion submission audit records and verifies the full identities, self-hashes, displayed estimands, raw error ledgers, and claim mappings. This index is descriptive provenance, not a new analysis or a broader release decision.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.15\linewidth}>{\raggedright\arraybackslash}p{0.17\linewidth}X>{\raggedright\arraybackslash}p{0.14\linewidth}@{}}
\toprule
\textbf{Stage} & \textbf{Scale} & \textbf{Principal frozen observation} & \textbf{File SHA-256 prefix} \\
\midrule
E1 controlled & 288 shards; 8,928 rows; 49 groups & Genuine $Q=16$ minus $Q=0$ accuracy gain 0.2460; matched-shuffled equivalence fails & \texttt{5edaf84a10b0} \\
Interface replay & 1,860 rows; 49 groups & Primary explicit-grounding contrast $-0.1781$; semantic framing displaces the exact schema & \texttt{3cf902ec9d97} \\
Paired readout & 2,976 passes; 1,488 pairs & Minimal genuine target and shuffled donor AUCs both 1.000; controls at chance & \texttt{6391ce62d0e6} \\
E2 controlled & 104 shards; 4,836 rows; 49 groups & Selection gain 0.3004 in all suites; effect gain 0.1553 but suite-heterogeneous & \texttt{ac162941f738} \\
Strict repair gate & 245 rows; 147 learned episodes & 0/147 learned episodes contract-valid; oracle succeeds 49/49 & \texttt{42fc7908ef04} \\
Native repair gate & 49 tasks; 123 model turns & 122/123 turns parse, but only 35/49 episodes are protocol-valid & \texttt{1b677b8fa3ef} \\
\bottomrule
\end{tabularx}
\caption{Final scientific-v6 train-only evidence index. Full artifact hashes and machine-checked claim bindings are reported in the companion submission audit.}
\label{tab:evidenceindex}
\end{table}
\FloatBarrier
...
Native tool chat solves learned repair & Turn parsing is 0.9919, but episode validity is 0.7443 and semantic tool errors dominate & Syntax improves; checkpoint/interface pair not qualified \\
Clean repair preserves useful work & Oracle succeeds 49/49; strict-JSON and native-Qwen feasibility gates fail before efficacy release & Open; not supported or refuted for a qualified repair process \\
...
\bottomrule
\end{tabularx}
\caption{Manuscript claim ledger after the frozen scientific-v6 E1, E2, and clean-repair feasibility analyses.}
...
The E1 audit, adaptive readouts, completed E2, and repair gates enforce a measurement distinction. \emph{Channel distinguishability} asks whether a source-aligned score can recover the state from the transcript. \emph{Posterior realization} asks whether the learned model's numeric state score ranks the states. \emph{Syntactic contract realization} asks whether the model emits the required schema, label namespace, or native tool syntax. \emph{Grounded tool execution} asks whether syntactically valid calls name real entities and satisfy the tool schema. \emph{Action transfer} asks whether the information changes the state-matched final choice. \emph{Effect realization} asks whether that choice becomes a released prohibited or useful benchmark effect. \emph{End-to-end realization} retains the conservative operational score in which invalid output receives its prespecified worst-case safety value. These quantities answer different questions and are reported separately; neither a post-hoc posterior, a forced-choice logit, a parsed tool call, nor a selected action silently replaces the frozen safety or utility endpoint.
...
For learned repair, task success is interpreted only after a prospective interface gate. Per-turn parse rate is diagnostic; the independent unit is the complete task episode. A checkpoint/interface pair must meet its frozen episode-validity point and confidence-bound thresholds before observed utility can authorize an efficacy study. This prevents malformed or semantically ungrounded calls from being silently dropped and prevents a favorable subset of valid episodes from defining the repair claim.
...
The repair studies add two further limitations. First, the strict-JSON run never executes a learned call, so its zero utility is an invalidity score rather than a semantic estimate. Second, native chat is evaluated only on Qwen2.5-7B, one tokenizer template, oracle-function restriction, and 49 train tasks. Its 99.19\% turn parse rate coexists with substantially lower episode validity because one bad identifier or argument invalidates the task; this gap may change with checkpoint capability, tool-error recovery, or interface policy. The four non-ASCII prompt-binding failures are a validator-serializer defect retained under the frozen rule. They lower run validity but do not explain failure of the primary protocol-validity gate. We therefore report them separately and do not use either their correction or the diagnostic native utility to release a new experiment.
...
\begin{enumerate}
    ...
    \item \textbf{Complete:} author and validate a 49-task clean-repair component with perfect released-oracle execution, then retain the negative strict-JSON learned-interface result without imputing utility;
    \item \textbf{Complete:} preregister and execute a native-tool checkpoint qualification, retain its failed episode-validity gate and non-ASCII serializer limitation, and stop before repair efficacy or development;
    \item \textbf{Complete:} run a model-free submission audit binding the final displayed E1, E2, adaptive, and repair numbers and the abstract/conclusion claims to the immutable train artifacts; and
    \item \textbf{Future work only:} consider another repair checkpoint, development protocol, held-out evaluation, stronger attacker, or fixed-monitor extension as a new immutable study with its own permission-bearing gate.
\end{enumerate}

The immediate scientific risk is no longer whether the transcript contains source-aligned evidence or whether that evidence can change a state-conditioned action. The remaining risks are whether the layered train-only mechanism generalizes beyond one model and authored intervention, whether a revised protocol can obtain permission-bearing evidence without weakening the failed controls, and whether any qualified fresh repair process preserves useful work. The current paper is feasible without answering those future questions: its empirical contribution is the E1/E2 mechanism chain, and its repair contribution is a prospective feasibility-gate result that prevents invalid outputs from becoming a selective utility claim. No scientific-v5 development, pair reduction, repair subset, or held-out job is reused under the revised claim.
...
The controlled pilot demonstrates that verdict-correlated feedback can produce both state inference and additional synthetic prohibited effects under a fixed opportunity budget. Scientific-v6 E1 shows a strong but heterogeneous learned-model end-to-end signal: Banking and Slack realize it under the exact contract, while Travel and Workspace often fail at the symbolic interface. The adaptive studies explain why that heterogeneity is not channel privacy. Under a paired minimal readout, genuine feedback tracks the target and shuffled feedback tracks only its donor with AUC $1.000$ in every suite; constant feedback remains at chance. Scientific-v6 E2 then shows that genuine feedback changes the state-matched final action in every suite, while only Banking and Slack carry that change through to the conservative prohibited-effect endpoint. The clean-repair gates expose the same distinction downstream: native chat almost eliminates malformed calls, but hallucinated entities and invalid arguments keep complete-episode validity below release. The suite split is therefore a layered mechanism result rather than a binary leakage/non-leakage result.

Under \method{}, hidden-state prediction still increases with probe budget, but state-matched selection and commitment do not, and the conservative prohibited-effect score decreases. Recipient separation therefore exhibits partial action-layer separation without satisfying the stronger empirical closure criterion. The frozen E1 placebo failure, E2 closure failure, and repair qualification failure keep every learned-model result train-only and keep development closed. The paper's strongest current contribution is both architectural and methodological: truthful feedback can be moved away from a potentially compromised context, and agent-security evaluation must distinguish information availability, syntax, grounded execution, action choice, and released effect. The prospective repair gate is part of that contribution because it records where the evidence chain stops instead of estimating utility on a selected valid subset. Useful learned repair and cross-model or held-out robustness remain future studies under new immutable protocols; they are not required to turn the completed train evidence into a coherent mechanism paper.

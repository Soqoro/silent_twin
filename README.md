...
The checked-in implementation is a deterministic Tier-1 finite-state testbed.
It runs without a GPU, model download, network access, or paid API. Tier 2 is an
extension boundary for realistic model clients and tool environments; it is not
silently emulated by the Tier-1 runner.
...
Python 3.10 or newer and Bash 4.3 or newer are required. The launchers prepend
`src/` to `PYTHONPATH`, so installation is optional for local runs. To install
...
|---|---|---|---|
| E1: feedback leakage | Decision-correlated feedback reveals hidden guardrail state; exact SilentTwin does not. | Runtime, attacker, query budget, world suite, seed | State-inference accuracy and lift over prior, transcript AUC, proxy-guardrail accuracy versus query budget |
...
Every experiment launcher supports `grid`, `run`, and `aggregate`. The default
stage is `run`, selected with `SLURM_ARRAY_TASK_ID`.
...
Each run directory contains `result.jsonl`, `manifest.json`, and `run.log`.
Results are written to a temporary file, validated, and atomically published.
The JSONL ends with exactly one summary record. Private state, monitor verdicts,
and ground-truth labels belong to trusted evaluation fields and are excluded
from the canonical agent-visible transcript.

See the detailed guides for the [research design](docs/research_design.md),
...
model, provider, operating system, or distributed tool stack. Do not report the
mock outputs as empirical LLM results. The repository never calls a paid API,
downloads a large model, or submits a SLURM job on its own.

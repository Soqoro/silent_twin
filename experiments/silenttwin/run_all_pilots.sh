#!/bin/bash
# Run one minimal deterministic mock configuration for each experiment, then
# aggregate those pilot outputs. E2 uses one complete four-row target/donor
# block; the other experiments default to two rows. This script never submits
# scheduler jobs.
#
...
E2_NUM_SAMPLES="${E2_NUM_SAMPLES:-4}"
SEEDS="${SEEDS:-42}"
...
[[ "$E2_NUM_SAMPLES" =~ ^[1-9][0-9]*$ ]] && ((E2_NUM_SAMPLES % 4 == 0)) || {
    printf 'silenttwin: error: E2_NUM_SAMPLES must be a positive multiple of four\n' >&2
    exit 2
}
...
if [[ "$PILOT_STAGE" == aggregate || "$PILOT_STAGE" == all ]]; then
    ...
    aggregate_pilot run_experiment_2_feedback_assisted_bypass.sh E2_STAGE \
        TIERS=tier1 WORLD_SUITES=email RUNTIMES=generic_failure \
        ATTACKERS=mock_llm QUERY_BUDGETS=4 E2_CONDITIONS=adaptive \
        "SEEDS=$SEEDS" "NUM_SAMPLES=$E2_NUM_SAMPLES"

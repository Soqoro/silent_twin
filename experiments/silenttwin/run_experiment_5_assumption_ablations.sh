...
main() {
    ...
    local -a grid_arguments=(--experiment e5 --num-samples "$NUM_SAMPLES")
    ...
    st_append_repeated grid_arguments --ablation ablations
    ...
    case "$ST_STAGE" in
        grid)
            ...
            ;;
        aggregate)
            local expected_manifest="$OUT_ROOT/e5/aggregate/grid_manifest.jsonl"

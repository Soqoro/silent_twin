...
# Experiment 1 asks whether agent-visible guardrail feedback reveals private state.
# Default grid: E1_STAGE=grid bash experiments/silenttwin/run_experiment_1_feedback_leakage.sh
# Tiny local smoke:
#   TIERS=tier1 WORLD_SUITES=email RUNTIMES=generic_failure ATTACKERS=mock_llm QUERY_BUDGETS=4 SEEDS=42 NUM_SAMPLES=2 SLURM_ARRAY_TASK_ID=0 bash experiments/silenttwin/run_experiment_1_feedback_leakage.sh
# Default SLURM array (384 jobs; create logs before submission):
#   mkdir -p logs
#   sbatch --array=0-383 experiments/silenttwin/run_experiment_1_feedback_leakage.sh
# Aggregate: E1_STAGE=aggregate bash experiments/silenttwin/run_experiment_1_feedback_leakage.sh
...
main() {
    ...
    case "$ST_STAGE" in
        ...
        run)
            ...
            local run_dir="$OUT_ROOT/e1/tier=$SELECTED_TIER/world=$SELECTED_WORLD/runtime=$SELECTED_RUNTIME/attacker=$SELECTED_ATTACKER/q=$SELECTED_BUDGET/seed=$SELECTED_SEED"
            local run_command=(

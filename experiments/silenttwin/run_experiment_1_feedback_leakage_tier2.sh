#!/bin/bash
#SBATCH --job-name=st-e1-tier2
#SBATCH --output=logs/st-e1-tier2-%A_%a.out
#SBATCH --error=logs/st-e1-tier2-%A_%a.err
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G

# Prepared-only Tier-2 entrypoint. This repository has no verified account,
# partition, or one-GPU request syntax, so those MUST be supplied to sbatch.
# The script deliberately has no guessed #SBATCH partition/account/GPU line.
#
...
case "$PILOT_PRESET" in
    ...
    "")
        [[ "$DATASET_SPLIT" == test && -n "$SAMPLE_SIZE_FREEZE" ]] || {
            printf '%s\n' 'silenttwin: error: custom Tier-2 E1 requires DATASET_SPLIT=test and SAMPLE_SIZE_FREEZE' >&2
            exit 2
        }
        EPISODES_PER_SHARD="${EPISODES_PER_SHARD:-20}"
        export EPISODES_PER_SHARD
        ;;
    *) printf '%s\n' 'silenttwin: error: Tier-2 E1 accepts pilot_c, pilot_d, or a frozen custom test grid' >&2; exit 2 ;;
esac
...
exec bash "$script_dir/run_experiment_1_feedback_leakage.sh" "$@"

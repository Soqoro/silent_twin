...
# E2 tests whether target-correlated feedback helps choose one monitor blind spot.
# Invalid condition/budget crosses are removed before hashing or array selection.
#   PILOT_PRESET=pilot_a E2_STAGE=grid bash experiments/silenttwin/run_experiment_2_feedback_assisted_bypass.sh
#   PILOT_PRESET=pilot_b E2_STAGE=grid bash experiments/silenttwin/run_experiment_2_feedback_assisted_bypass.sh
...
build_grid_arguments() {
    ...
    st_append_repeated grid_arguments --condition conditions
...
run_selected_cell() {
    local cell_name="$1"
    local -n cell_ref="$cell_name"
    # Hash-address every result so model/decoding, pair/template, split, and
    # held-out-freeze factors cannot collide in valid custom grids.
    local run_dir="$OUT_ROOT/e2/pilot=${cell_ref[pilot_id]:-custom}/config=${cell_ref[configuration_hash]}"
    local run_command=(
...
main() {
    ...
    E2_CONDITIONS="${E2_CONDITIONS:-no_probe adaptive random oracle shuffled}"
    SEEDS="${SEEDS:-42}"
    ...
    st_validate_choices E2_CONDITIONS 'no_probe adaptive genuine random oracle shuffled' "${conditions[@]}"
    st_validate_choices PAIR_FAMILIES 'monitor_blind_spot blind_spot' "${pair_families[@]}"
    ...
    case "$ST_STAGE" in
        grid)
            ...
            ;;
        aggregate)
            local expected_manifest="$OUT_ROOT/e2/aggregate/grid_manifest.jsonl"
